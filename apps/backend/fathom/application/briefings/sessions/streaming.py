...
from fathom.application.briefings.sessions.event_coordinator import (
    JobEventCoordinator,
...
STREAM_CONTROL_TICK_SECONDS = 1.0
FALLBACK_RECONCILE_SECONDS = 45.0
...
async def stream_briefing_session_events(
    ...
    *,
    ...
    event_coordinator: JobEventCoordinator | None = None,
) -> AsyncIterator[str]:
    ...
    async with managed_supabase_client(await create_supabase_admin_client(settings)) as admin_client:
        stream_lease_token = await claim_stream_lease(
            ...
            max_per_user=SSE_MAX_STREAMS_PER_USER,
            max_per_subject=SSE_MAX_STREAMS_PER_IP,
            lease_seconds=SSE_STREAM_LEASE_SECONDS,
        )
...
async def session_event_stream_with_clients(
    *,
    ...
    async with optional_event_subscription(event_coordinator, session_id_str) as subscription:
        ...
        keepalive_due_at = stream_started_at + KEEPALIVE_SECONDS
        fallback_reconcile_due_at = stream_started_at + FALLBACK_RECONCILE_SECONDS
        lease_renewed_at = stream_started_at
        lease_renewal_interval = max(10.0, SSE_STREAM_LEASE_SECONDS / 3)

        while True:
            now = time.monotonic()
            if now - stream_started_at >= SSE_STREAM_MAX_LIFETIME_SECONDS:
                with log_context(user_id=auth.user_id, session_id=session_id_str):
            ...
            update = await wait_for_event_update(
                subscription=subscription,
                user_client=user_client,
                session_id=session_id_str,
                cursor=cursor,
                fallback_reconcile_due_at=fallback_reconcile_due_at,
            )
            now = time.monotonic()
            if subscription is None and now >= fallback_reconcile_due_at:
                fallback_reconcile_due_at = now + FALLBACK_RECONCILE_SECONDS

            if update is not None:
                for event in update.events:
                    sequence_id = event_sequence_id(event)
                    if sequence_id is None or sequence_id <= cursor:
                        continue
                    cursor = sequence_id
                    yield encode_replayed_event(event, sequence_id)

                if update.reconcile_snapshot:
                    cursor = max(cursor, update.latest_sequence)
            ...
            now = time.monotonic()
            if now >= keepalive_due_at:
                yield ": keepalive\n\n"
                keepalive_due_at = now + KEEPALIVE_SECONDS


@asynccontextmanager
async def optional_event_subscription(
    coordinator: JobEventCoordinator | None,
    session_id: str,
) -> AsyncIterator[JobEventSubscription | None]:
    if coordinator is None:
        yield None
        return
    async with coordinator.subscribe(session_id, cursor=0) as subscription:
        yield subscription


async def wait_for_event_update(
    *,
    subscription: JobEventSubscription | None,
    user_client: Any,
    session_id: str,
    cursor: int,
    fallback_reconcile_due_at: float,
) -> JobEventUpdate | None:
    if subscription is not None:
        return await subscription.wait(STREAM_CONTROL_TICK_SECONDS)

    await asyncio.sleep(STREAM_CONTROL_TICK_SECONDS)
    if time.monotonic() < fallback_reconcile_due_at:
        return None

    # This path is only used when the API lifespan coordinator is unavailable,
    # such as a direct unit invocation. It remains slow and durable rather than
    # restoring the previous one-query-per-viewer-per-second behavior.
    ...
    latest_sequence = await fetch_latest_job_event_sequence(user_client, job_id=session_id)
    return JobEventUpdate(
        events=tuple(events),
        latest_sequence=latest_sequence,
