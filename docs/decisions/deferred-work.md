...
| Object-storage provider choice, including Cloudflare R2 | Keep private Supabase Storage for temporary audio and signed PDF downloads | R2 may remove temporary-audio egress to the transcription provider or reduce PDF download egress, but moving now adds another credential, adapter, signed-URL policy, cleanup path, CORS policy, and recovery surface before real cost is known | Measured temporary-audio/PDF egress, reliability, recovery needs, or Supabase limits become material | Evaluate temporary audio and PDFs separately; private bucket; short-lived signed URLs; server-mediated authorization; least-privilege credentials; CORS; object cleanup/retention; migration and rollback proof; measured total cost |
