...
| Polar sandbox contract | Billing transactions, signature checks, replay safety, ordering, recovery, and trusted-proxy customer-IP forwarding are covered by code and disposable tests | On a taxable sandbox order, verify the provider's total-versus-net refund amount contract; verify hosted proxy resolution and the resulting country/currency/tax inference | External proof needed before paid launch |
