...
This has a safe default and normally stays unset in the personal `.env`:
...
This remains deployment configuration because the safe concurrency depends on
the worker machine and provider capacity.

Source metadata uses a 30-second code timeout, source audio download uses a
300-second code timeout, each Groq transcription attempt uses 120 seconds, and
each OpenRouter summary attempt uses 300 seconds. Provider calls make at most
three attempts with bounded backoff. The billing debt cap is 600 seconds. SSE
allows three streams per user and twelve per client IP, leases them for 90
seconds, and closes each stream after 3,600 seconds so the browser reconnects.
Worker shutdown uses a 30-second code grace period. Backend logs use console
output for `local` and `test`, and structured JSON for `staging` and
`production`, derived from `APP_ENV`.
